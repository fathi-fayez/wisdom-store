<template>
  <div>
    <v-navigation-drawer class="cart-drawer px-5 pt-4" v-model="drawer" width="370" temporary>
      <v-card class="px-0" elevation="0">
        <v-card-title class="px-0 pr-2 d-flex justify-space-between align-center w-100"
          style="font-size: 17px; font-weight: bold">Menu
          <v-icon @click="drawer = false">mdi-close</v-icon></v-card-title>
        <v-list>
          <v-list-item v-for="cat in categories" :key="cat.route" @click="
            $router.push({
              name: 'categoryPage', params: { category: cat.route, title: cat.title }
            })
          ...
          <v-list-group>
            ...
            <v-list-item>
              <v-list-item-title class="d-flex align-center ga-2"> <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                  x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512; width: 20px"
...
const drawer = ref<any>(isMenuOpen)
</script>
